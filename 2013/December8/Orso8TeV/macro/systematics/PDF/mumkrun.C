...
using namespace std;*/
{
	TString p1 = "./Syst2J1T dirName TreesMu dopdf ";
	TString p2 = " input ";
	TString p3 = " METResolFileName /home/nadjieh/work/AnalysisClasses/ToyAnalysis/macro/METresolutions.root isData no  stack yes ";
	TString p4 = " > TreesMu_";
	TString p5 = ".txt\n";
	...
	for(int s = 1; s < 53; s++){
	    ...
	    for(unsigned int i = 0; i < prefix.size(); i++){
			...
			if(prefix[i] == " prefix mutau ")
				myfile<<" ttDecayMode mutau ";
			if(prefix[i] == " prefix mue ")
				myfile<<" ttDecayMode mue ";
			if(prefix[i] == " prefix dimu ")
				myfile<<" ttDecayMode dimu ";
			myfile<<endl;
    	}
		myfile.close();
	}
