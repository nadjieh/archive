...
int main(int argc, char** argv) {
    ...
    for (int f = 1; f < argc; f++) {
        ...
        if (arg_fth == "F0Up") {
            ...
            f++;
            std::string in(*(argv + f));
            FlStat = atof(in.c_str());
        } else if (arg_fth == "FrStat") {
            f++;
            std::string in(*(argv + f));
            FrStat = atof(in.c_str());
        } else if (arg_fth == "F0Stat") {
            f++;
            std::string in(*(argv + f));
            F0Stat = atof(in.c_str());
        } else if (arg_fth == "StatCorr") {
            f++;
            std::string in(*(argv + f));
            StatCorr = atof(in.c_str());
        }
    }
    ...
    if (FrStat != 0 && FlStat != 0 && F0Stat != 0 && StatCorr != 0) {
        FrUp.push_back(FrStat);
        FrDown.push_back(FrStat);
        myIndividualSyst.AddUncertainty(F0Stat, 1, "Up");
        myIndividualSyst.AddUncertainty(F0Stat, 1, "Down");
        myIndividualSyst.AddUncertainty(FlStat, 2, "Up");
        myIndividualSyst.AddUncertainty(FlStat, 2, "Down");
        myIndividualSyst.AddCorrelation(StatCorr);
        myIndividualSyst.AddName("stat.");
    }
