...
Event myEventPreparation(MuonTree * input) {
    ...
    BJET.setBtag_combinedSecondaryVertexBJetTags(input->highBTag);
    nonBJET.setBtag_combinedSecondaryVertexBJetTags(input->lowBTag);
//#if defined TOP && defined ISDATA
//    BJET = input->GetFJet();
//    nonBJET = input->GetBJet();
//    BJET.setBtag_combinedSecondaryVertexBJetTags(input->lowBTag);
//    nonBJET.setBtag_combinedSecondaryVertexBJetTags(input->highBTag);
//#endif
    TRootMuon MUON(input->GetLepton());
    TRootPFMET MET(input->GetMET());
    TRootVertex VTX;
...
int main(int argc, char** argv) {
    ...
    for (unsigned int fNumber = 0; fNumber < inputFileNames.size(); fNumber++) {
...
#ifdef ISDATA
#if !defined QCD && !defined Wtemplate !defined TOP
...
#endif /*Wtemplate*/
#if defined TOP  && !defined Wtemplate && !defined QCD
        myMuonTree = new MuonTree(eventTree, f, sample + "_2J_2T_noSyst", dirName);
#endif /*TOP*/
#if defined TOP  && !defined Wtemplate && defined QCD
        myMuonTree = new MuonTree(eventTree, f, sample + "_2J_2T_QCD_noSyst", dirName);
#endif /*TOP*/
#endif /*ISDATA*/
#if !defined ISDATA && !defined Wtemplate
        if (prefix != string("")) {
        ...
        for (int eventNumber = 0; eventNumber < myMuonTree->fChain->GetEntriesFast(); eventNumber++) {
            ...
            if (!myMuonTree->passExtraSelection(applyJetPt)) {
                continue;
            }
...
#if !defined ISDATA && !defined Wtemplate && !defined TOP
